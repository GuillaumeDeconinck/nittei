...
public class CalendarApplicationService {
  ...
  public ResponseEntity<CalendarApi.CalendarResponse> createCalendarAdmin(
      HttpHeaders headers, String userId, CalendarApi.CreateCalendarRequestBody body) {
  ...
  public ResponseEntity<CalendarApi.GetCalendarsByUserAPIResponse> getCalendarsAdmin(
      HttpHeaders headers, String userId, CalendarApi.GetCalendarsByUserQueryParams query) {
    var account = auth.requireAdminAccount(headers, ctx);
    ID uid = auth.parseId(userId, "Malformed userId");
    auth.requireAccountUser(account, uid, ctx);
    ...
    return ResponseEntity.ok(
        new CalendarApi.GetCalendarsByUserAPIResponse(
  ...
  public ResponseEntity<CalendarApi.GetCalendarsByMetaAPIResponse> getCalendarsByMetaAdmin(
      HttpHeaders headers, CalendarApi.GetCalendarsByMetaQueryParams query) {
    var account = auth.requireAdminAccount(headers, ctx);
        ...
        ctx.repos()
            ...
            .findByMetadata(
                new MetadataFindQuery(
                    ...
                    account.id()));
    return ResponseEntity.ok(
        new CalendarApi.GetCalendarsByMetaAPIResponse(
            calendars.stream().map(CalendarDtos.CalendarDTO::from).toList()));
  }

  public ResponseEntity<CalendarApi.CalendarResponse> getCalendarAdmin(
      HttpHeaders headers, String calendarId) {
    var account = auth.requireAdminAccount(headers, ctx);
    Calendar calendar =
        auth.requireAccountCalendar(account, auth.parseId(calendarId, "Malformed calendarId"), ctx);
  ...
  public ResponseEntity<CalendarApi.CalendarResponse> updateCalendarAdmin(
      HttpHeaders headers, String calendarId, CalendarApi.UpdateCalendarRequestBody body) {
    var account = auth.requireAdminAccount(headers, ctx);
    Calendar current =
        auth.requireAccountCalendar(account, auth.parseId(calendarId, "Malformed calendarId"), ctx);
    Calendar updated =
        new Calendar(
            ...
            body != null && body.settings() != null
                ? new CalendarSettings(
                    body.settings().weekStart() == null
                        ? current.settings().weekStart()
                        : body.settings().weekStart(),
                    body.settings().timezone() == null
  ...
  public ResponseEntity<CalendarApi.GetCalendarEventsAPIResponse> getCalendarEventsAdmin(
      HttpHeaders headers, String calendarId, CalendarApi.GetCalendarEventsQueryParams query) {
    var account = auth.requireAdminAccount(headers, ctx);
    Calendar calendar =
        auth.requireAccountCalendar(account, auth.parseId(calendarId, "Malformed calendarId"), ctx);
    var events =
        getCalendarEvents(calendar.id(), query).stream()
            .map(this::toEventWithSingleInstance)
            .toList();
  ...
  public ResponseEntity<String> exportCalendarIcalAdmin(
      HttpHeaders headers, String calendarId, CalendarApi.GetCalendarEventsIcalQueryParams query) {
    var account = auth.requireAdminAccount(headers, ctx);
    Calendar calendar =
        auth.requireAccountCalendar(account, auth.parseId(calendarId, "Malformed calendarId"), ctx);
    List<CalendarEvent> events =
        getCalendarEvents(
            calendar.id(),
            new CalendarApi.GetCalendarEventsQueryParams(query.startTime(), query.endTime()));
  ...
  public ResponseEntity<CalendarApi.GetGoogleCalendarsAPIResponse> getGoogleCalendarsAdmin(
      HttpHeaders headers, String userId, CalendarApi.GetGoogleCalendarsQueryParams query) {
    var account = auth.requireAdminAccount(headers, ctx);
    ID uid = auth.parseId(userId, "Malformed userId");
    ...
    try {
      calendars =
          providerCalendarService.listGoogleCalendars(
              user, query == null ? null : query.minAccessRole(), ctx);
    } catch (Exception e) {
      throw new NitteiApiException(
          NitteiErrorCode.INTERNAL_ERROR, "Unable to list google calendars");
    }
    if (calendars == null) {
      throw new NitteiApiException(
          NitteiErrorCode.BAD_CLIENT_DATA, "The user is not connected to google.");
    }
  ...
  public ResponseEntity<CalendarApi.GetOutlookCalendarsAPIResponse> getOutlookCalendarsAdmin(
      HttpHeaders headers, String userId, CalendarApi.GetOutlookCalendarsQueryParams query) {
    var account = auth.requireAdminAccount(headers, ctx);
    ID uid = auth.parseId(userId, "Malformed userId");
    var user = auth.requireAccountUser(account, uid, ctx);
    ...
    try {
      calendars =
          providerCalendarService.listOutlookCalendars(
              user, query == null ? null : query.minAccessRole(), ctx);
    } catch (Exception e) {
      throw new NitteiApiException(
          NitteiErrorCode.INTERNAL_ERROR, "Unable to list outlook calendars");
    }
    if (calendars == null) {
      throw new NitteiApiException(
          NitteiErrorCode.BAD_CLIENT_DATA, "The user is not connected to outlook.");
    }
  ...
  public ResponseEntity<CalendarApi.CalendarResponse> addSyncCalendarAdmin(
      HttpHeaders headers, String userId, CalendarApi.AddSyncCalendarRequestBody body) {
    var account = auth.requireAdminAccount(headers, ctx);
    ID uid = auth.parseId(userId, "Malformed userId");
    auth.requireAccountUser(account, uid, ctx);
    ...
    ctx.repos()
        .calendarSynced()
        .insert(new SyncedCalendar(body.provider(), calendar.id(), uid, body.extCalendarId()));
    return ResponseEntity.ok(CalendarApi.CalendarResponse.from(calendar));
  }

  public ResponseEntity<CalendarApi.CalendarResponse> removeSyncCalendarAdmin(
      HttpHeaders headers, String userId, CalendarApi.RemoveSyncCalendarRequestBody body) {
    var account = auth.requireAdminAccount(headers, ctx);
    ID uid = auth.parseId(userId, "Malformed userId");
    auth.requireAccountUser(account, uid, ctx);
    Calendar calendar = auth.requireAccountCalendar(account, body.calendarId(), ctx);
    ctx.repos()
        .calendarSynced()
        .delete(new SyncedCalendar(body.provider(), calendar.id(), uid, body.extCalendarId()));
    return ResponseEntity.ok(CalendarApi.CalendarResponse.from(calendar));
  }

  public ResponseEntity<CalendarApi.CalendarResponse> createCalendar(
      HttpHeaders headers, CalendarApi.CreateCalendarRequestBody body) {
  ...
  public ResponseEntity<CalendarApi.GetCalendarsByUserAPIResponse> getCalendars(
      HttpHeaders headers, CalendarApi.GetCalendarsByUserQueryParams query) {
    var userCtx = auth.requireUserContext(headers, ctx);
  ...
  public ResponseEntity<CalendarApi.CalendarResponse> getCalendar(
    ...
    Calendar calendar =
        ctx.repos()
            .calendars()
            .find(auth.parseId(calendarId, "Malformed calendarId"))
            .filter(c -> c.userId().equals(userCtx.user().id()))
            .orElseThrow(
                () ->
                    new NitteiApiException(
                        NitteiErrorCode.NOT_FOUND,
                        "The calendar with id: " + calendarId + ", was not found"));
    return ResponseEntity.ok(CalendarApi.CalendarResponse.from(calendar));
  }
  ...
  public ResponseEntity<CalendarApi.GetGoogleCalendarsAPIResponse> getGoogleCalendars(
      HttpHeaders headers, CalendarApi.GetGoogleCalendarsQueryParams query) {
    var userCtx = auth.requireUserContext(headers, ctx);
    List<com.meetsmore.nittei.domain.providers.google.GoogleCalendarListEntry> calendars;
    try {
      calendars =
          providerCalendarService.listGoogleCalendars(
              userCtx.user(), query == null ? null : query.minAccessRole(), ctx);
    } catch (Exception e) {
      throw new NitteiApiException(
          NitteiErrorCode.INTERNAL_ERROR, "Unable to list google calendars");
    }
  ...
  public ResponseEntity<CalendarApi.GetOutlookCalendarsAPIResponse> getOutlookCalendars(
      HttpHeaders headers, CalendarApi.GetOutlookCalendarsQueryParams query) {
    var userCtx = auth.requireUserContext(headers, ctx);
    List<com.meetsmore.nittei.domain.providers.outlook.OutlookCalendar> calendars;
    try {
      calendars =
  ...
  private Calendar buildCalendar(
      ID userId, ID accountId, CalendarApi.CreateCalendarRequestBody body) {
    DayOfWeek weekStart =
        body == null || body.weekStart() == null ? DayOfWeek.MONDAY : body.weekStart();
    String timezone = body == null || body.timezone() == null ? "UTC" : body.timezone();
    try {
      ZoneId.of(timezone);
    } catch (Exception e) {
      throw new NitteiApiException(
          NitteiErrorCode.UNPROCESSABLE_ENTITY, "failed to parse timezone: '" + timezone + "'");
    }
  ...
  private List<CalendarEvent> getCalendarEvents(
      ID calendarId, CalendarApi.GetCalendarEventsQueryParams query) {
    ...
    return ctx.repos()
        .events()
        .findByCalendar(calendarId, new TimeSpan(query.startTime(), query.endTime()));
  }

  private EventWithInstances toEventWithSingleInstance(CalendarEvent event) {
    return new EventWithInstances(
        event, List.of(new EventInstance(event.startTime(), event.endTime(), event.busy())));
  }
