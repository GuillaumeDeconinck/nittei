...
public class RequestContextResolver {
    ...
    public Account requireAdminAccount(HttpHeaders headers, NitteiContext ctx) {
        ...
        return ctx.repos().accounts().findByApiKey(apiKey)
            .orElseThrow(() -> new NitteiApiException(NitteiErrorCode.UNAUTHORIZED, "Invalid x-api-key header"));
    }
    ...
    public Account requirePublicAccount(HttpHeaders headers, NitteiContext ctx) {
        ...
        return ctx.repos().accounts().find(accountId)
            .orElseThrow(() -> new NitteiApiException(NitteiErrorCode.UNIDENTIFIABLE_CLIENT, "Could not identify account"));
    }
    ...
    public UserContext requireUserContext(HttpHeaders headers, NitteiContext ctx) {
        ...
        User user = ctx.repos().users().findByAccountId(userId, account.id())
            .orElseThrow(() -> new NitteiApiException(NitteiErrorCode.UNAUTHORIZED, "Unable to find user from credentials"));

        return new UserContext(account, user);
    }

    public User requireAccountUser(Account account, ID userId, NitteiContext ctx) {
        User user = ctx.repos().users().find(userId)
            .orElseThrow(() -> new NitteiApiException(NitteiErrorCode.NOT_FOUND, "A user with id: " + userId.value() + ", was not found"));
        if (!account.id().equals(user.accountId())) {
            throw new NitteiApiException(NitteiErrorCode.NOT_FOUND, "A user with id: " + userId.value() + ", was not found");
        }
        return user;
    }

    public Calendar requireAccountCalendar(Account account, ID calendarId, NitteiContext ctx) {
        Calendar calendar = ctx.repos().calendars().find(calendarId)
            .orElseThrow(() -> new NitteiApiException(NitteiErrorCode.NOT_FOUND, "The calendar with id: " + calendarId.value() + ", was not found"));
        if (!account.id().equals(calendar.accountId())) {
            throw new NitteiApiException(NitteiErrorCode.NOT_FOUND, "The calendar with id: " + calendarId.value() + ", was not found");
        }
        return calendar;
    }

    public Schedule requireAccountSchedule(Account account, ID scheduleId, NitteiContext ctx) {
        Schedule schedule = ctx.repos().schedules().find(scheduleId)
            .orElseThrow(() -> new NitteiApiException(NitteiErrorCode.NOT_FOUND, "Schedule not found"));
    ...
    public CalendarEvent requireAccountEvent(Account account, ID eventId, NitteiContext ctx) {
        CalendarEvent event = ctx.repos().events().find(eventId)
        ...
        if (!account.id().equals(event.accountId())) {
            throw new NitteiApiException(NitteiErrorCode.NOT_FOUND, "The calendar event with id: " + eventId.value() + ", was not found");
        }
        return event;
    }

    public Service requireAccountService(Account account, ID serviceId, NitteiContext ctx) {
        Service service = ctx.repos().services().find(serviceId)
    ...
    private ID resolveUserIdFromJwt(HttpHeaders headers, Account account) {
        ...
        if (authHeader == null || authHeader.isBlank() || !authHeader.toLowerCase().startsWith("bearer ")) {
            throw new NitteiApiException(NitteiErrorCode.UNAUTHORIZED, "Missing or invalid Authorization header");
        }
        ...
        try {
            ...
            String userId = JWT.require(Algorithm.RSA256(publicKey, null))
                .build()
                .verify(token)
    ...
    private RSAPublicKey toRsaPublicKey(Account account) throws Exception {
        if (account.publicJwtKey() == null || account.publicJwtKey().value() == null || account.publicJwtKey().value().isBlank()) {
            throw new NitteiApiException(NitteiErrorCode.UNAUTHORIZED, "Account does not support user tokens");
        }
        String pem = account.publicJwtKey().value()
