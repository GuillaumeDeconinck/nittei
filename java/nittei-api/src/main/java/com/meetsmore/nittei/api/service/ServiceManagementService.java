...
public class ServiceManagementService {
  ...
  public ResponseEntity<ServiceApi.ServiceResponse> createService(
      HttpHeaders headers, ServiceApi.CreateServiceRequestBody body) {
  ...
  public ResponseEntity<ServiceApi.GetServicesByMetaAPIResponse> getServicesByMeta(
      HttpHeaders headers, ServiceApi.GetServicesByMetaQueryParams query) {
    var account = auth.requireAdminAccount(headers, ctx);
    var services =
        ctx
            .repos()
            ...
            .findByMetadata(
                new MetadataFindQuery(
                    ...
                    account.id()))
            .stream()
            .map(ServiceDtos.ServiceDTO::from)
            .toList();
  ...
  public ResponseEntity<ServiceDtos.ServiceWithUsersDTO> getService(
      HttpHeaders headers, String serviceId) {
    var account = auth.requireAdminAccount(headers, ctx);
  ...
  public ResponseEntity<ServiceApi.ServiceResponse> updateService(
      HttpHeaders headers, String serviceId, ServiceApi.UpdateServiceRequestBody body) {
    var account = auth.requireAdminAccount(headers, ctx);
    Service current =
        auth.requireAccountService(account, auth.parseId(serviceId, "Malformed serviceId"), ctx);
    Service updated =
        new Service(
            current.id(),
            current.accountId(),
  ...
  public ResponseEntity<ServiceApi.ServiceResourceResponse> addUserToService(
      HttpHeaders headers, String serviceId, ServiceApi.AddUserToServiceRequestBody body) {
    var account = auth.requireAdminAccount(headers, ctx);
    ID sid = auth.parseId(serviceId, "Malformed serviceId");
    auth.requireAccountService(account, sid, ctx);
  ...
  public ResponseEntity<ServiceApi.ServiceResourceResponse> removeUserFromService(
      HttpHeaders headers, String serviceId, String userId) {
    var account = auth.requireAdminAccount(headers, ctx);
    ID sid = auth.parseId(serviceId, "Malformed serviceId");
    auth.requireAccountService(account, sid, ctx);
    ID uid = auth.parseId(userId, "Malformed userId");
    ServiceResource current =
        ctx.repos()
            .serviceUsers()
            .find(sid, uid)
            .orElseThrow(
                () -> new NitteiApiException(NitteiErrorCode.NOT_FOUND, "Service user not found"));
  ...
  public ResponseEntity<ServiceApi.ServiceResourceResponse> updateServiceUser(
    ...
    ServiceResource updated =
        new ServiceResource(
            ...
            body.closestBookingTime() == null
                ? current.closestBookingTime()
                : body.closestBookingTime(),
            body.furthestBookingTime() == null
                ? current.furthestBookingTime()
                : body.furthestBookingTime());
  ...
  public ResponseEntity<ServiceApi.ServiceResourceResponse> addBusyCalendar(
    ...
    ctx.repos()
        ...
        .orElseThrow(
            () -> new NitteiApiException(NitteiErrorCode.NOT_FOUND, "Service user not found"));
    if ("google".equalsIgnoreCase(body.busy().provider())
        || "outlook".equalsIgnoreCase(body.busy().provider())) {
      ctx.repos()
          .serviceUserBusyCalendars()
          .insertExternal(
              new ExternalBusyCalendarIdentifier(
                  sid, uid, body.busy().id(), parseProvider(body.busy().provider())));
    } else {
      ctx.repos()
          .serviceUserBusyCalendars()
          .insert(
              new BusyCalendarIdentifier(
                  sid, uid, auth.parseId(body.busy().id(), "Malformed busy calendar id")));
    }
