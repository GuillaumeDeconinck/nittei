...
public class AccountApplicationService {
    ...
    public ResponseEntity<AccountApi.CreateAccountResponseBody> createAccount(AccountApi.CreateAccountRequestBody body) {
    ...
    public ResponseEntity<AccountApi.AccountResponse> getAccount(HttpHeaders headers) {
        ...
        Account fresh = ctx.repos().accounts().find(account.id())
            .orElseThrow(() -> new NitteiApiException(NitteiErrorCode.NOT_FOUND, "Account not found"));
    ...
    public ResponseEntity<AccountApi.AccountResponse> setAccountPubKey(
        HttpHeaders headers,
        AccountApi.SetAccountPubKeyRequestBody body
    ) {
        Account account = auth.requireAdminAccount(headers, ctx);
        PEMKey key = body == null || body.publicJwtKey() == null ? null : new PEMKey(body.publicJwtKey());
    ...
    public ResponseEntity<AccountApi.AccountResponse> setAccountWebhook(
        HttpHeaders headers,
        AccountApi.SetAccountWebhookRequestBody body
    ) {
        Account account = auth.requireAdminAccount(headers, ctx);
        if (body == null || body.webhookUrl() == null || !body.webhookUrl().startsWith("https://")) {
            throw new NitteiApiException(NitteiErrorCode.BAD_CLIENT_DATA, "Malformed url or scheme is not https");
        }
        AccountSettings settings = new AccountSettings(new AccountWebhookSettings(body.webhookUrl(), null));
        Account updated = new Account(account.id(), account.secretApiKey(), account.publicJwtKey(), settings);
        ctx.repos().accounts().save(updated);
        return ResponseEntity.ok(AccountApi.AccountResponse.from(updated));
    }
    ...
    public ResponseEntity<AccountApi.AccountResponse> addAccountIntegration(
        HttpHeaders headers,
        AccountApi.AddAccountIntegrationRequestBody body
    ) {
        Account account = auth.requireAdminAccount(headers, ctx);
        if (body == null || body.provider() == null) {
            throw new NitteiApiException(NitteiErrorCode.BAD_CLIENT_DATA, "Missing provider");
        }
        boolean exists = ctx.repos().accountIntegrations().find(account.id()).stream().anyMatch(i -> i.provider() == body.provider());
        if (exists) {
            throw new NitteiApiException(NitteiErrorCode.CONFLICT, "Account already has an integration for that provider");
        }
        ctx.repos().accountIntegrations().insert(
            new AccountIntegration(account.id(), body.clientId(), body.clientSecret(), body.redirectUri(), body.provider())
        );
    ...
    public ResponseEntity<AccountApi.AccountResponse> removeAccountIntegration(
        ...
        boolean exists = ctx.repos().accountIntegrations().find(account.id()).stream().anyMatch(i -> i.provider() == p);
        if (!exists) {
            throw new NitteiApiException(NitteiErrorCode.NOT_FOUND, "Did not find integration for provider");
        }
    ...
    public ResponseEntity<AccountApi.SearchEventsAPIResponse> accountSearchEvents(
        HttpHeaders headers,
        AccountApi.AccountSearchEventsRequestBody body
    ) {
        Account account = auth.requireAdminAccount(headers, ctx);
        AccountApi.AccountSearchEventsRequestBodyFilter filter = body == null ? null : body.filter();
        SearchEventsParams params = new SearchEventsParams(
        ...
        List<EventDtos.CalendarEventDTO> events =
            ctx.repos().events().searchEventsForAccount(
            ...
            ).stream().map(EventDtos.CalendarEventDTO::from).toList();
