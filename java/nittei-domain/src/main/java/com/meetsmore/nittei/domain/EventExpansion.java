...
public final class EventExpansion {
    ...
    public static List<EventInstance> toCompatibleInstances(List<EventInstance> instances) {
        ...
        for (EventInstance current : sorted) {
            ...
            if (previous.busy() == current.busy()
                ...
                Instant mergedStart = previous.startTime().isBefore(current.startTime()) ? previous.startTime() : current.startTime();
                Instant mergedEnd = previous.endTime().isAfter(current.endTime()) ? previous.endTime() : current.endTime();
    ...
    private static Map<ID, List<Instant>> generateExceptionsMap(List<CalendarEvent> events) {
        ...
        for (CalendarEvent event : events) {
            if (event.recurringEventId() != null && event.originalStartTime() != null) {
                exceptions.computeIfAbsent(event.recurringEventId(), ignored -> new ArrayList<>())
    ...
    private static List<EventInstance> expandEvent(Calendar calendar, CalendarEvent event, TimeSpan timespan) {
        ...
        ZoneId zoneId = ZoneId.of(calendar.settings() == null || calendar.settings().timezone() == null
            ? "UTC"
            : calendar.settings().timezone());

        RRuleOptions recurrence = event.recurrence();
        int interval = recurrence.interval() == null || recurrence.interval() <= 0 ? 1 : recurrence.interval();
        ...
        while (generated < MAX_INSTANCES) {
            ...
            if (!occurrenceStart.isBefore(timespan.start()) && !occurrenceStart.isAfter(timespan.end())) {
                if (event.exdates() == null || !event.exdates().contains(occurrenceStart)) {
                    results.add(new EventInstance(occurrenceStart, occurrenceStart.plusMillis(computeDurationMs(event)), event.busy()));
                }
            }
