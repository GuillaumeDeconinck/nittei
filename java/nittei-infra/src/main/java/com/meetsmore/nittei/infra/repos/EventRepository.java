...
public interface EventRepository {
    ...
    List<CalendarEvent> findByRecurringEventIdsForTimespan(List<ID> recurringEventIds, TimeSpan timespan);
    ...
    List<CalendarEvent> findEventsForUsersForTimespan(List<ID> userIds, TimeSpan timespan, boolean includeTentative, boolean includeNonBusy);
    List<CalendarEvent> findRecurringEventsForUsersForTimespan(List<ID> userIds, TimeSpan timespan, boolean includeTentative, boolean includeNonBusy);
    List<CalendarEvent> findByCalendars(List<ID> calendarIds, TimeSpan timespan);
    List<CalendarEvent> findBusyEventsAndRecurringEventsForCalendars(List<ID> calendarIds, TimeSpan timespan, boolean includeTentative);
    ...
    List<MostRecentCreatedServiceEvents> findMostRecentlyCreatedServiceEvents(ID serviceId, List<ID> userIds);
    List<CalendarEvent> findByService(ID serviceId, List<ID> userIds, Instant minTime, Instant maxTime);
    List<CalendarEvent> findUserServiceEvents(ID userId, boolean busy, Instant minTime, Instant maxTime);
