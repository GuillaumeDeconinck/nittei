...
public class OutlookOAuthService {
    ...
    private static final String TOKEN_REFETCH_ENDPOINT = "https://login.microsoftonline.com/common/oauth2/v2.0/token";
    private static final String CODE_TOKEN_EXCHANGE_ENDPOINT = "https://login.microsoftonline.com/common/oauth2/v2.0/token";
    private static final List<String> REQUIRED_OAUTH_SCOPES = List.of(
    ...
    public String getAccessToken(User user, NitteiContext context) {
        ...
        var outlookSettings = context.repos().accountIntegrations().find(user.accountId()).stream()
            .filter(i -> i.provider() == IntegrationProvider.OUTLOOK)
            .findFirst()
            .orElse(null);
        ...
        Map<String, Object> refreshed = webClient.post()
            ...
            .body(BodyInserters.fromFormData("client_id", outlookSettings.clientId())
