...
public class PostgresServiceRepository implements ServiceRepository {
    ...
    public Optional<Service> find(ID serviceId) {
        List<Service> rows = jdbcTemplate.query(
    ...
    public Optional<ServiceWithUsers> findWithUsers(ID serviceId) {
        ...
        List<ServiceResource> users = jdbcTemplate.query(
        ...
        return Optional.of(new ServiceWithUsers(s.id(), s.accountId(), users, s.multiPerson(), s.metadata()));
    }
    ...
    private Service mapService(ResultSet rs) throws SQLException {
        try {
            ...
            ServiceMultiPersonOptions multiPerson = new ServiceMultiPersonOptions(
                String.valueOf(mp.getOrDefault("variant", "collective")),
                mp.get("data")
            );
